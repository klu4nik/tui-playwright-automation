...
export class SearchPanel {
    ...
    readonly departureActiveAirportOptions: Locator;
    ...
    /**
     * Opens the departure airport picker and selects a specific airport by name.
     *
     * @param airport - The departure airport name (must match visible text)
     */
    async selectDepartureAirport(airport: string): Promise<string> {
        return test.step(`Select departure airport: ${airport}`, async () => {
            await this.departureAirportField.first().click();
    ...
    async selectRandomDestination(): Promise<string> {
        return test.step('Select a random destination', async () => {
            ...
            console.log(`✓ Departure countries placeholder: ${countryOptions}`);
    ...
    /**
     * Opens the destination picker and selects a specific country and airport.
     * If airport is not provided, a random available airport in the country is chosen.
     *
     * @param country - The destination country name (must match visible text)
     * @param airport - Optional airport name; if omitted, picks a random one
     */
    async selectDestination(country: string, airport?: string): Promise<string> {
        return test.step(`Select destination: ${country}${airport ? ` / ${airport}` : ''}`, async () => {
            await this.destinationAirportSelectionButton.first().click();
            await expect(this.destinationCountrySelectableOptions.first()).toBeVisible();
